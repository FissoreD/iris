...
From myiris.algebra Require Export ofe monoid.
...
Elpi Accumulate TC.Solver lp:{{ tc-myiris.algebra.cmra.tc-PCore _ {{cmra_pcore _}}. }}.
Global Hint Extern 0 (Op _) => refine (cmra_op _); shelve : typeclass_instances.
Elpi Accumulate TC.Solver lp:{{ tc-myiris.algebra.cmra.tc-Op _ {{cmra_op _}}. }}.
Global Hint Extern 0 (Valid _) => refine (cmra_valid _); shelve : typeclass_instances.
Elpi Accumulate TC.Solver lp:{{ tc-myiris.algebra.cmra.tc-Valid _ {{cmra_valid _}}. }}.
Global Hint Extern 0 (ValidN _) => refine (cmra_validN _); shelve : typeclass_instances.
Elpi Accumulate TC.Solver lp:{{ tc-myiris.algebra.cmra.tc-ValidN _ {{cmra_validN _}}. }}.
...
Elpi Accumulate TC.Solver lp:{{ tc-myiris.algebra.cmra.tc-Unit _ {{ucmra_unit _}}. }}.
...
Elpi Accumulate TC.Solver lp:{{
  tc-myiris.algebra.cmra.tc-CmraMorphism X X R S :-
    ID = {{@id _}}, not (R  = ID),
    coq.unify-eq R ID ok,
    tc-myiris.algebra.cmra.tc-CmraMorphism X X ID S.
}}.
...
Section discrete.
  ...
  Qed.
  Elpi Accumulate TC.Solver lp:{{
    tc-myiris.algebra.ofe.tc-OfeDiscrete T R :-
      T' = app [{{@Ofe}}, A, B, X, Y],
...
(* 
  % TODO: here the problem linked to this:
  % https://coq.zulipchat.com/#narrow/dm/299356,598022-dm/near/448849484
*)
  Goal exists A B, (cmra_ofeO prodR) = prodO A B. do 2 eexists. reflexivity. Qed.
  (* Note that this accumulate does not solve the following problem... why? *)
  Elpi Accumulate TC.Solver lp:{{
    tc-myiris.algebra.ofe.tc-OfeDiscrete X S :-
      Y = {{@prodO _ _}}, not (X = Y), 
      coq.unify-eq X Y ok,
      tc-myiris.algebra.ofe.tc-OfeDiscrete Y S.
  }}.
...
Section prod_unit.
  ...
  (* The following problem can be solve by simplifying first the goal *)
  Goal CoreId ε.1. simpl. apply _. (* ucmra_unit_core_id *) Qed.
...
Qed.
(* Here again we need to simpl the goal *)
Next Obligation.
  simpl.
  apply _.
Qed.
...
Elpi Accumulate TC.Solver lp:{{
  ...
  tc-myiris.algebra.cmra.tc-CmraMorphism A B X S :-
    A' = {{@prodR _ _}}, not (A = A'), 
    B' = {{@prodR _ _}}, not (B = B'), 
    coq.unify-eq A A' ok,
    coq.unify-eq B B' ok,
...
Section option.
  ...
  Proof.
    do 2 eexists.
    (* TODO: here coq.unify-eq not working 
      coq.unify-eq 
          (app [global (const «cmra_car»), global (const «A»)]) 
          (app [primitive (proj iris.algebra.ofe.ofe_car 0), X463]) ok
    *)
    Elpi Query A lp:{{
      coq.typecheck X {{ofe}} ok,
      coq.unify-eq {{cmra_car A}} {{(lp:X).(ofe_car)}} Err.
    }}.

    Fail Elpi Query A lp:{{
      coq.unify-eq {{cmra_car A}} {{(lp:X).(ofe_car)}} ok.
    }}.

    Fail apply _. (* Some_dist_inj *)
  Abort.
  ...
  Elpi Accumulate TC.Solver lp:{{ print-goal-pp. print-solution. }}.
  Goal CmraDiscrete A → True ->  OfeDiscrete (cmra_ofeO optionR).
    intros.
    Elpi Trace Browser.
    apply _.
    (* apply option_ofe_discrete. *)
    apply H.
  Qed.
  ...
  Local Instance option_unit_instance : Unit (option A) := None.
    Elpi Accumulate TC.Solver lp:{{ print-goal-pp. print-solution-pp. }}.
