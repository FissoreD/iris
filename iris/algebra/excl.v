...
Definition exclO_map `{SI : indexT} {A B} (f : A -n> B) : exclO A -n> exclO B :=
  OfeMor (excl_map f).
Global Instance exclO_map_ne `{SI : indexT} A B : NonExpansive (@exclO_map SI A B).
...
Global Instance exclRF_contractive `{SI : indexT} F :
