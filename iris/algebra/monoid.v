...
Class Monoid `{SI : indexT} {M : ofe} (o : M → M → M) := {
  monoid_unit : M;
...
Lemma monoid_proper `{SI : indexT} `{Monoid M o} : Proper ((≡) ==> (≡) ==> (≡)) o.
Proof. apply ne_proper_2, monoid_ne. Qed.
Lemma monoid_right_id `{SI : indexT} `{Monoid M o} : RightId (≡) monoid_unit o.
...
Class WeakMonoidHomomorphism `{SI : !indexT} {M1 M2 : ofe}
...
Class MonoidHomomorphism `{SI : !indexT} {M1 M2 : ofe}
    (o1 : M1 → M1 → M1) (o2 : M2 → M2 → M2) `{!Monoid o1, !Monoid o2}
    (R : relation M2) (f : M1 → M2) := {
