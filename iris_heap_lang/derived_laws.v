...
Proof. by rewrite /array big_sepLZ_singleton loc_add_0. Qed.

Lemma array_appZ l dq vs ws :
  l ↦∗{dq} (vs ++ ws) ⊣⊢ l ↦∗{dq} vs ∗ (l +ₗ lengthZ vs) ↦∗{dq} ws.
Proof.
  rewrite /array big_sepLZ_app.
  by setoid_rewrite loc_add_assoc.
Qed.
Lemma array_app l dq vs ws :
  l ↦∗{dq} (vs ++ ws) ⊣⊢ l ↦∗{dq} vs ∗ (l +ₗ Z.of_nat (length vs)) ↦∗{dq} ws.
Proof. apply array_appZ. Qed.

Lemma array_cons l dq v vs : l ↦∗{dq} (v :: vs) ⊣⊢ l ↦{dq} v ∗ (l +ₗ 1) ↦∗{dq} vs.
Proof.
  rewrite /array big_sepLZ_cons loc_add_0.
...
Lemma update_arrayZ l dq vs (off : Z) v :
  vs !! off = Some v →
  ⊢ l ↦∗{dq} vs -∗ ((l +ₗ off) ↦{dq} v ∗ ∀ v', (l +ₗ off) ↦{dq} v' -∗ l ↦∗{dq} <[off:=v']>vs).
Proof.
  iIntros (Hlookup) "Hl".
  rewrite -[X in (l ↦∗{_} X)%I](takeZ_dropZ_middle _ off v); last done.
  iDestruct (array_appZ with "Hl") as "[Hl1 Hl]".
  iDestruct (array_cons with "Hl") as "[Hl2 Hl3]".
  assert (0 ≤ off < lengthZ vs)%Z as H by (apply lookupZ_lt_is_Some; by eexists).
  rewrite takeZ_lengthZ Z.min_l; last by lia. rewrite Z.max_l; last by lia. iFrame "Hl2".
  iIntros (w) "Hl2".
  clear Hlookup. assert (<[off:=w]> vs !! off = Some w) as Hlookup.
  { apply list_lookupZ_insertZ. lia. }
  rewrite -[in (l ↦∗{_} <[off:=w]> vs)%I](takeZ_dropZ_middle (<[off:=w]> vs) off w Hlookup).
  iApply array_appZ. rewrite takeZ_insertZ; last by lia. iFrame.
  iApply array_cons. rewrite takeZ_lengthZ Z.min_l; last by lia. rewrite Z.max_l; last by lia. iFrame.
  rewrite dropZ_insertZ_gt; last by lia. done.
Qed.
Lemma update_array l dq vs (off : nat) v :
  vs !! off = Some v →
  ⊢ l ↦∗{dq} vs -∗ ((l +ₗ Z.of_nat off) ↦{dq} v ∗ ∀ v', (l +ₗ Z.of_nat off) ↦{dq} v' -∗ l ↦∗{dq} <[off:=v']>vs).
Proof. intros. setoid_rewrite <-insertZ_of_nat. apply update_arrayZ. rewrite lookupZ_of_nat. done. Qed.

(** * Rules for allocation *)
Lemma mapsto_seqZ_array l dq v n :
  ([∗ list] i ∈ seqZ 0 n, (l +ₗ i) ↦{dq} v) -∗
  l ↦∗{dq} replicateZ n v.
Proof. rewrite /array. rewrite big_sepLZ_replicateZ_seqZ. done. Qed.
Lemma mapsto_seq_array l dq v n :
  ([∗ list] i ∈ seq 0 n, (l +ₗ (Z.of_nat i)) ↦{dq} v) -∗
  l ↦∗{dq} replicate n v.
Proof.
  rewrite seq_seqZ big_sepL_fmap. iIntros "H".
  iEval replace n with (Z.to_nat (Z.of_nat n)) by lia.
  iApply mapsto_seqZ_array. iApply (big_sepL_impl with "H").
  iIntros "!#" (?? Hseq%lookup_seqZ). rewrite Z2Nat.id; last by lia. auto.
Qed.

Lemma twp_allocNZ s E v n :
  (0 < n)%Z →
  [[{ True }]] AllocN (Val $ LitV $ LitInt $ n) (Val v) @ s; E
  [[{ l, RET LitV (LitLoc l); l ↦∗ replicateZ n v ∗
         [∗ list] i ∈ seqZ 0 n, meta_token (l +ₗ i) ⊤ }]].
Proof.
  iIntros (Hzs Φ) "_ HΦ". iApply twp_allocN_seqZ; [done..|].
  iIntros (l) "Hlm". iApply "HΦ".
  iDestruct (big_sepL_sep with "Hlm") as "[Hl $]".
  by iApply mapsto_seqZ_array.
Qed.
Lemma wp_allocNZ s E v n :
  (0 < n)%Z →
  {{{ True }}} AllocN (Val $ LitV $ LitInt $ n) (Val v) @ s; E
  {{{ l, RET LitV (LitLoc l); l ↦∗ replicateZ n v ∗
         [∗ list] i ∈ seqZ 0 n, meta_token (l +ₗ i) ⊤ }}}.
Proof.
  ...
  iApply twp_allocNZ; [auto..|]; iIntros (l) "H HΦ". by iApply "HΦ".
Qed.
...
Lemma twp_allocN s E v n :
  ...
  [[{ l, RET LitV (LitLoc l); l ↦∗ replicate (Z.to_nat n) v ∗
         [∗ list] i ∈ seq 0 (Z.to_nat n), meta_token (l +ₗ (Z.of_nat i)) ⊤ }]].
Proof.
  iIntros (? Φ) "_ HΦ". iApply twp_allocNZ; [done..|].
  iIntros (l) "[Hl Hmeta]". iApply "HΦ". iFrame "Hl".
  rewrite seqZ_seq; last by lia. rewrite big_sepL_fmap. done.
Qed.
...
Lemma twp_load_offsetZ s E l dq (off : Z) vs v :
  vs !! off = Some v →
  [[{ l ↦∗{dq} vs }]] ! #(l +ₗ off) @ s; E [[{ RET v; l ↦∗{dq} vs }]].
Proof.
  iIntros (Hlookup Φ) "Hl HΦ".
  iDestruct (update_arrayZ l _ _ _ _ Hlookup with "Hl") as "[Hl1 Hl2]".
  iApply (twp_load with "Hl1"). iIntros "Hl1". iApply "HΦ".
  iDestruct ("Hl2" $! v) as "Hl2". rewrite list_insertZ_id; last done.
  iApply "Hl2". iApply "Hl1".
Qed.
Lemma wp_load_offsetZ s E l dq (off : Z) vs v :
  vs !! off = Some v →
  {{{ ▷ l ↦∗{dq} vs }}} ! #(l +ₗ off) @ s; E {{{ RET v; l ↦∗{dq} vs }}}.
Proof.
  iIntros (? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_load_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ".
  by iApply "HΦ".
Qed.

Lemma twp_load_offset s E l dq (off : nat) vs v :
  vs !! off = Some v →
  [[{ l ↦∗{dq} vs }]] ! #(l +ₗ Z.of_nat off) @ s; E [[{ RET v; l ↦∗{dq} vs }]].
Proof. rewrite -lookupZ_of_nat. apply twp_load_offsetZ. Qed.
Lemma wp_load_offset s E l dq (off : nat) vs v :
  vs !! off = Some v →
  {{{ ▷ l ↦∗{dq} vs }}} ! #(l +ₗ Z.of_nat off) @ s; E {{{ RET v; l ↦∗{dq} vs }}}.
Proof. rewrite -lookupZ_of_nat. apply wp_load_offsetZ. Qed.

Lemma twp_load_offset_vec s E l dq sz (off : fin sz) (vs : vec val sz) :
  [[{ l ↦∗{dq} vs }]] ! #(l +ₗ Z.of_nat off) @ s; E [[{ RET vs !!! off; l ↦∗{dq} vs }]].
...
Lemma twp_store_offsetZ s E l (off : Z) vs v :
  is_Some (vs !! off) →
...
Qed.
Lemma wp_store_offsetZ s E l (off : Z) vs v :
  is_Some (vs !! off) →
  {{{ ▷ l ↦∗ vs }}} #(l +ₗ off) <- v @ s; E {{{ RET #(); l ↦∗ <[off:=v]> vs }}}.
Proof.
  iIntros (? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_store_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ".
  by iApply "HΦ".
Qed.

Lemma twp_store_offset s E l (off : nat) vs v :
  is_Some (vs !! off) →
  [[{ l ↦∗ vs }]] #(l +ₗ Z.of_nat off) <- v @ s; E [[{ RET #(); l ↦∗ <[off:=v]> vs }]].
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply twp_store_offsetZ. Qed.
Lemma wp_store_offset s E l (off : nat) vs v :
  is_Some (vs !! off) →
  {{{ ▷ l ↦∗ vs }}} #(l +ₗ Z.of_nat off) <- v @ s; E {{{ RET #(); l ↦∗ <[off:=v]> vs }}}.
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply wp_store_offsetZ. Qed.

Lemma twp_store_offset_vec s E l sz (off : fin sz) (vs : vec val sz) v :
  [[{ l ↦∗ vs }]] #(l +ₗ Z.of_nat off) <- v @ s; E [[{ RET #(); l ↦∗ vinsert off v vs }]].
Proof.
...
Lemma twp_xchg_offsetZ s E l (off : Z) vs v v' :
  vs !! off = Some v →
...
Proof.
  ...
  iApply "HΦ". iApply "Hl2". iApply "Hl1".
Qed.
Lemma wp_xchg_offsetZ s E l (off : Z) vs v v' :
  vs !! off = Some v →
  {{{ ▷ l ↦∗ vs }}} Xchg #(l +ₗ off) v' @ s; E {{{ RET v; l ↦∗ <[off:=v']> vs }}}.
Proof.
  iIntros (? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_xchg_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ".
  by iApply "HΦ".
Qed.

Lemma twp_xchg_offset s E l (off : nat) vs v v' :
  vs !! off = Some v →
  [[{ l ↦∗ vs }]] Xchg #(l +ₗ Z.of_nat off) v' @ s; E [[{ RET v; l ↦∗ <[off:=v']> vs }]].
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply twp_xchg_offsetZ. Qed.
Lemma wp_xchg_offset s E l (off : nat) vs v v' :
  vs !! off = Some v →
  {{{ ▷ l ↦∗ vs }}} Xchg #(l +ₗ Z.of_nat off) v' @ s; E {{{ RET v; l ↦∗ <[off:=v']> vs }}}.
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply wp_xchg_offsetZ. Qed.

Lemma twp_xchg_offset_vec s E l sz (off : fin sz) (vs : vec val sz) v :
  [[{ l ↦∗ vs }]] Xchg #(l +ₗ Z.of_nat off) v @ s; E [[{ RET (vs !!! off); l ↦∗ vinsert off v vs }]].
Proof.
...
Lemma twp_cmpxchg_suc_offsetZ s E l (off : Z) vs v' v1 v2 :
  vs !! off = Some v' →
  v' = v1 →
...
Qed.
Lemma wp_cmpxchg_suc_offsetZ s E l (off : Z) vs v' v1 v2 :
  vs !! off = Some v' →
  v' = v1 →
  vals_compare_safe v' v1 →
  {{{ ▷ l ↦∗ vs }}}
...
Proof.
  iIntros (??? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_cmpxchg_suc_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ".
  by iApply "HΦ".
Qed.

Lemma twp_cmpxchg_suc_offset s E l (off : nat) vs v' v1 v2 :
  vs !! off = Some v' →
  v' = v1 →
  vals_compare_safe v' v1 →
  [[{ l ↦∗ vs }]]
    CmpXchg #(l +ₗ Z.of_nat off) v1 v2 @ s; E
  [[{ RET (v', #true); l ↦∗ <[off:=v2]> vs }]].
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply twp_cmpxchg_suc_offsetZ. Qed.
Lemma wp_cmpxchg_suc_offset s E l (off : nat) vs v' v1 v2 :
  vs !! off = Some v' →
  v' = v1 →
  vals_compare_safe v' v1 →
  {{{ ▷ l ↦∗ vs }}}
    CmpXchg #(l +ₗ Z.of_nat off) v1 v2 @ s; E
  {{{ RET (v', #true); l ↦∗ <[off:=v2]> vs }}}.
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply wp_cmpxchg_suc_offsetZ. Qed.
...
Lemma twp_cmpxchg_fail_offsetZ s E l dq (off : Z) vs v0 v1 v2 :
  vs !! off = Some v0 →
  v0 ≠ v1 →
...
Proof.
  ...
  rewrite list_insertZ_id; last done. iApply "Hl2". iApply "Hl1".
Qed.
Lemma wp_cmpxchg_fail_offsetZ s E l dq (off : Z) vs v0 v1 v2 :
  vs !! off = Some v0 →
  v0 ≠ v1 →
  vals_compare_safe v0 v1 →
  {{{ ▷ l ↦∗{dq} vs }}}
...
Proof.
  iIntros (??? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_cmpxchg_fail_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ".
  by iApply "HΦ".
Qed.

Lemma twp_cmpxchg_fail_offset s E l dq (off : nat) vs v0 v1 v2 :
  vs !! off = Some v0 →
  v0 ≠ v1 →
  vals_compare_safe v0 v1 →
  [[{ l ↦∗{dq} vs }]]
    CmpXchg #(l +ₗ Z.of_nat off) v1 v2 @ s; E
  [[{ RET (v0, #false); l ↦∗{dq} vs }]].
Proof. rewrite -lookupZ_of_nat. apply twp_cmpxchg_fail_offsetZ. Qed.
Lemma wp_cmpxchg_fail_offset s E l dq (off : nat) vs v0 v1 v2 :
  vs !! off = Some v0 →
  v0 ≠ v1 →
  vals_compare_safe v0 v1 →
  {{{ ▷ l ↦∗{dq} vs }}}
    CmpXchg #(l +ₗ Z.of_nat off) v1 v2 @ s; E
  {{{ RET (v0, #false); l ↦∗{dq} vs }}}.
Proof. rewrite -lookupZ_of_nat. apply wp_cmpxchg_fail_offsetZ. Qed.
...
Lemma twp_faa_offsetZ s E l (off : Z) vs (i1 i2 : Z) :
  vs !! off = Some #i1 →
...
Proof.
  ...
  iIntros "Hl1". iApply "HΦ". iApply "Hl2". iApply "Hl1".
Qed.
Lemma wp_faa_offsetZ s E l (off : Z) vs (i1 i2 : Z) :
  vs !! off = Some #i1 →
  {{{ ▷ l ↦∗ vs }}} FAA #(l +ₗ off) #i2 @ s; E
  {{{ RET LitV (LitInt i1); l ↦∗ <[off:=#(i1 + i2)]> vs }}}.
Proof.
  iIntros (? Φ) ">H HΦ". iApply (twp_wp_step with "HΦ").
  iApply (twp_faa_offsetZ with "H"); [by eauto..|]; iIntros "H HΦ". by iApply "HΦ".
Qed.

Lemma twp_faa_offset s E l (off : nat) vs (i1 i2 : Z) :
  vs !! off = Some #i1 →
  [[{ l ↦∗ vs }]] FAA #(l +ₗ Z.of_nat off) #i2 @ s; E
  [[{ RET LitV (LitInt i1); l ↦∗ <[off:=#(i1 + i2)]> vs }]].
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply twp_faa_offsetZ. Qed.
Lemma wp_faa_offset s E l (off : nat) vs (i1 i2 : Z) :
  vs !! off = Some #i1 →
  {{{ ▷ l ↦∗ vs }}} FAA #(l +ₗ Z.of_nat off) #i2 @ s; E
  {{{ RET LitV (LitInt i1); l ↦∗ <[off:=#(i1 + i2)]> vs }}}.
Proof. rewrite -lookupZ_of_nat -insertZ_of_nat. apply wp_faa_offsetZ. Qed.
