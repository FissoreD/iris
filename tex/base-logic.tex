...
Note that the modalities $\upd$, $\persistently$, $\plainly$ and $\later$ bind more tightly than $*$, $\wand$, $\land$, $\lor$, and $\Ra$.
...
\and
	\infer{\vctx \proves \wtt{\prop}{\Prop}}
		{\vctx \proves \wtt{\persistently\prop}{\Prop}}
\and
	\infer{\vctx \proves \wtt{\prop}{\Prop}}
...
%Furthermore, an arbitrary \emph{boxed} proposition context $\persistently\pfctx$ may be added to every constituent.
...
\and
\infer[$\plainly$-E]{}
{\plainly\prop \proves \persistently\prop}
\and
\begin{array}[c]{rMcMl}
...
\begin{mathpar}
\inferhref{$\persistently$-mono}{pers-mono}
  {\prop \proves \propB}
  {\persistently{\prop} \proves \persistently{\propB}}
\and
\inferhref{$\persistently$-E}{pers-elim}
{}
{\persistently\prop \proves \prop}
\and
\begin{array}[c]{rMcMl}
  (\plainly P \Ra \persistently Q) &\proves& \persistently (\plainly P \Ra Q) \\
  \persistently{\prop} \land \propB &\proves& \persistently{\prop} * \propB
\end{array}
\and
\begin{array}[c]{rMcMl}
  \persistently{\prop} &\proves& \persistently\persistently\prop \\
  \All x. \persistently{\prop} &\proves& \persistently{\All x. \prop} \\
  \persistently{\Exists x. \prop} &\proves& \Exists x. \persistently{\prop}
\end{array}
\end{mathpar}
...
\ownM\melt &\proves& \persistently{\ownM{\mcore\melt}} \\
...
For $\persistently$ and $\plainly$, this follows from the elimination rules.
