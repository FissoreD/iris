...
\begin{align*}
  ...
  \prop \vs[\mask_1][\mask_2] \propB \eqdef{}& \persistently(\prop \wand \pvs[\mask_1][\mask_2] \propB) \\
...
\and
\inferHB{vs-always}
  {\persistently\propB \proves \prop \vs[\mask_1][\mask_2] \propC}
  {\prop \land \persistently{\propB} \vs[\mask_1][\mask_2] \propC}
 \and
\inferH{vs-false}
  {}
...
\[
\hoare{\prop}{\expr}{\Ret\val.\propB}[\mask] \eqdef \persistently{(\prop \wand \wpre{\expr}[\mask]{\Ret\val.\propB})}
\]
...
\and
\inferHB{Ht-box}
  {\persistently\propB \proves \hoare{\prop}{\expr}{\Ret\val.\propC}[\mask]}
  {\hoare{\prop \land \persistently{\propB}}{\expr}{\Ret\val.\propC}[\mask]}
\end{mathparpagebreakable}
...
\begin{mathpar}
  \axiomH{inv-persist}{\invM\namesp\prop \proves \persistently\invM\namesp\prop}
...
\begin{align*}
  \knowInv\namesp\prop \eqdef \persistently\All\mask. \pvs[\mask][\mask\setminus\namesp] \later\prop * (\later\prop \vsW[\mask\setminus\namesp][\mask] \TRUE)
\end{align*}
...
\begin{mathpar}
  ...
  \inferH{inv-alter}
  {}
  {\later\persistently(\prop \wand \propB * (\propB \wand \prop)) \vdash \knowInv\namesp\prop \wand \knowInv\namesp\propB}
\end{mathpar}
