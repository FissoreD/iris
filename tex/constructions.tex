...
This is what we do for option $\optionm\cofe$, product $(M_i)_{i \in I}$ (with $I$ some finite index set), sum $\cofe + \cofe'$ and finite partial functions $K \fpfn \monoid$ (with $K$ infinite countable).
...
\end{align*}
Remember that $\mnocore$ is the ``dummy'' element in $\optionm\monoid$ indicating (in this case) that $\exinj(x)$ has no core.
...
\begin{align*}
\authm(M) \eqdef{}& \optionm{\exm(M)} \times M \\
...
\begin{defn}
  ...
  \[ \All n, \optionm{\melt_\f}. n \in \mval(\melt_1) \land \melt_1 \dist{n} \meltB_1 \mtimes \optionm{\melt_\f} \Ra n \in \mval(\melt_2) \land \melt_2 \dist{n} \meltB_2 \mtimes \optionm{\melt_\f} \]
\end{defn}
In other words, the idea is that for every possible frame $\optionm{\melt_\f}$ completing $\meltB_1$ to $\melt_1$, the same frame also completes $\meltB_2$ to $\melt_2$.
