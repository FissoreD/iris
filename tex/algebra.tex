...
\begin{defn}
  An \emph{ordered family of equivalences} (OFE) is a tuple $(\ofe, ({\equivN{n}} \subseteq \ofe \times \ofe)_{n \in \nat})$ satisfying
  \begin{align*}
    \All n. (\equivN{n}) ~& \text{is an equivalence relation} \tagH{ofe-equiv} \\
    \All n, m.& n \geq m \Ra (\equivN{n}) \subseteq (\equivN{m}) \tagH{ofe-mono} \\
    \All x, y.& x = y \Lra (\All n. x \equivN{n} y) \tagH{ofe-limit}
  \end{align*}
\end{defn}

The key intuition behind OFEs is that elements $x$ and $y$ are $n$-equivalent, notation $x \equivN{n} y$, if they are \emph{equivalent for $n$ steps of computation}, \ie if they cannot be distinguished by a program running for no more than $n$ steps.
In other words, as $n$ increases, $\equivN{n}$ becomes more and more refined (\ruleref{ofe-mono})---and in the limit, it agrees with plain equality (\ruleref{ofe-limit}).
...
\begin{defn}
  ...
  \[ \All y \in \ofe. x \equivN{0} y \Ra x = y\]
  An OFE $A$ is called \emph{discrete} if all its elements are discrete.
  For a set $X$, we write $\Delta X$ for the discrete OFE with $x \equivN{n} x' \eqdef x = x'$
\end{defn}

\begin{defn}
  A function $f : \ofe \to \ofeB$ between two OFEs is \emph{non-expansive} (written $f : \ofe \nfn \ofeB$) if
  \[\All n, x \in \ofe, y \in \ofe. x \equivN{n} y \Ra f(x) \equivN{n} f(y) \]
  It is \emph{contractive} if
  \[ \All n, x \in \ofe, y \in \ofe. (\All m < n. x \equivN{m} y) \Ra f(x) \equivN{n} f(y) \]
\end{defn}
...
\begin{defn}
  ...
  \begin{align*}
    f \equivN{n} g \eqdef{}& \All x \in \ofe. f(x) \equivN{n} g(x)
  \end{align*}
\end{defn}
...
\begin{align*}
  ...
  X \equivN{n} Y \eqdef{}& \All m \leq n. m \in X \Lra m \in Y \\
  X \inclN{n} Y \eqdef{}& \All m \leq n. m \in X \Ra m \in Y
\end{align*}
...
\begin{defn}
  ...
  \begin{align*}
    ...
    \All n, \melt, \meltB_1, \meltB_2.& \omit\rlap{$n \in \mval(\melt) \land \melt \equivN{n} \meltB_1 \mtimes \meltB_2 \Ra {}$} \\
    &\Exists \meltC_1, \meltC_2. \melt = \meltC_1 \mtimes \meltC_2 \land \meltC_1 \equivN{n} \meltB_1 \land \meltC_2 \equivN{n} \meltB_2 \tagH{camera-extend} \\
    \text{where}\qquad\qquad\\
    \melt \mincl \meltB \eqdef{}& \Exists \meltC. \meltB = \melt \mtimes \meltC \tagH{camera-incl} \\
    \melt \mincl[n] \meltB \eqdef{}& \Exists \meltC. \meltB \equivN{n} \melt \mtimes \meltC \tagH{camera-inclN}
  \end{align*}
\end{defn}
...
% RJ FIXME: Needs some magic to fix the baseline of the $\equivN{n}$, or so
\begin{center}
\begin{tikzpicture}[every edge/.style={draw=none}]
  ...
  \path (a) edge node {$\equivN{n}$} (b);
  \path (a12) edge node {$\equivN{n}$} (b12);
  \path (a) edge node [rotate=90] {$=$} (a12);
  \path (b) edge node [rotate=90] {$=$} (b12);
\end{tikzpicture}\end{center}
where the $n$-equivalence at the bottom is meant to apply to the pairs of elements, \ie we demand $\melt_1 \equivN{n} \meltB_1$ and $\melt_2 \equivN{n} \meltB_2$.
