...
\begin{mathpar}
  \inferH{CInv-new}{}
  {\later\prop \vs[\bot] \Exists \gname. \CInvTok\gname{1} * \persistently\CInv\gname\namesp\prop}
...
\begin{align*}
  ...
  \BoxM \eqdef{}& \authm(\optionm{\exm(\BoxState)}) \times \optionm{\agm(\latert \iProp)}
\end{align*}
...
% Note that $x \fgmapsto v$ is duplicable but cannot be boxed (as it depends on resources); \ie we have $x \fgmapsto v \Lra x \fgmapsto v * x \fgmapsto v$ but not $x \fgmapsto v \Ra \persistently x \fgmapsto v$.
